...
provenance_path="$1"
env=$(cat "${provenance_path}" | jq -r '.dsseEnvelope.payload' | base64 -d | jq -r '.predicate.buildDefinition.internalParameters')
...
export GITHUB_REF_NAME=$(echo "${GITHUB_REF}" | cut -d '/' -f3)
